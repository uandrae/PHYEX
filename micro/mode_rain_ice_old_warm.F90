...
!
    ...
    IF (BUCONF%LBUDGET_RC) CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'AUTO', ZWKBUD)
    ...
    IF (BUCONF%LBUDGET_RC) ZWKBUD(:,:) = UNPACK(ZRCS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RC) CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'AUTO', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = UNPACK(ZRRS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%END_PHY(D, 'AUTO', ZWKBUD)

!*       4.3    compute the accretion of r_c for r_r production: RCACCR

    IF (BUCONF%LBUDGET_RC) ZWKBUD(:,:) = UNPACK(ZRCS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RC) CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'ACCR', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = UNPACK(ZRRS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%INIT_PHY(D, 'ACCR', ZWKBUD)
    ...
    IF (BUCONF%LBUDGET_RC) ZWKBUD(:,:) = UNPACK(ZRCS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RC) CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'ACCR', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = UNPACK(ZRRS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%END_PHY(D, 'ACCR', ZWKBUD)

!*       4.4    compute the evaporation of r_r: RREVAV

    IF (BUCONF%LBUDGET_TH) ZWKBUD(:,:) = UNPACK(ZTHS(:),MASK=GMICRO(:,:),FIELD=PTHS)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_TH) CALL TBUDGETS(NBUDGET_TH)%PTR%INIT_PHY(D, 'REVA', ZWKBUD)
    IF (BUCONF%LBUDGET_RV) ZWKBUD(:,:) = UNPACK(ZRVS(:),MASK=GMICRO(:,:),FIELD=PRVS)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RV) CALL TBUDGETS(NBUDGET_RV)%PTR%INIT_PHY(D, 'REVA', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = UNPACK(ZRRS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%INIT_PHY(D, 'REVA', ZWKBUD)
    ...
    IF (BUCONF%LBUDGET_TH) ZWKBUD(:,:) = UNPACK(ZTHS(:),MASK=GMICRO(:,:),FIELD=PTHS)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_TH) CALL TBUDGETS(NBUDGET_TH)%PTR%END_PHY(D, 'REVA', ZWKBUD)
    IF (BUCONF%LBUDGET_RV) ZWKBUD(:,:) = UNPACK(ZRVS(:),MASK=GMICRO(:,:),FIELD=PRVS)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RV) CALL TBUDGETS(NBUDGET_RV)%PTR%END_PHY(D, 'REVA', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = UNPACK(ZRRS(:)*ZRHODJ(:),MASK=GMICRO(:,:),FIELD=0.0)
    IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%END_PHY(D, 'REVA', ZWKBUD)
    DO JK=1, KSIZE
