...
DO JL=1, KSIZE
  IF (LDMICRO(JL)) THEN
    ZRCRAUTC(JL)=LIMAP%XCRIAUTC/PRHODREF(JL) ! Autoconversion rc threshold
  ELSE
    ZRCRAUTC(JL)=0.
  END IF
END DO
...
IF(HSUBG_AUCV_RC=='NONE') THEN
 ...
 DO JL=1, KSIZE
    IF (PRCT(JL)>ZRCRAUTC(JL) .AND. LDMICRO(JL)) THEN
      ...
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
    ELSE IF (PRCT(JL)>LIMAP%XRTMIN(2) .AND. LDMICRO(JL)) THEN
      PHLC_HCF(JL)=0.
      PHLC_LCF(JL)=1.
      PHLC_HRC(JL)=0.
      PHLC_LRC(JL)=PRCT(JL)
...
ELSEIF(HSUBG_AUCV_RC=='CLFR') THEN
 ...
 DO JL=1, KSIZE
    IF (PCF(JL)>0. .AND. PRCT(JL)>ZRCRAUTC(JL)*PCF(JL) .AND. LDMICRO(JL)) THEN
      ...
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRCT(JL)>LIMAP%XRTMIN(2) .AND. LDMICRO(JL)) THEN
      PHLC_HCF(JL)=0.
      PHLC_LCF(JL)=PCF(JL)
      PHLC_HRC(JL)=0.0
      PHLC_LRC(JL)=PRCT(JL)
...
ELSEIF(HSUBG_AUCV_RC=='PDF') THEN
  ...
  IF(HSUBG_PR_PDF=='SIGM') THEN
    ...
    DO JL=1, KSIZE
      ...
      ELSE IF (PRCT(JL)>LIMAP%XRTMIN(2) .AND. PCF(JL)>0. .AND. LDMICRO(JL)) THEN
        PHLC_HCF(JL)=0.
        PHLC_LCF(JL)=PCF(JL)
        PHLC_HRC(JL)=0.
        PHLC_LRC(JL)=PRCT(JL)
...
DO JL=1, KSIZE
  IF (LDMICRO(JL)) THEN
    ZCRIAUTI(JL)=MIN(LIMAP%XCRIAUTI,10**(LIMAP%XACRIAUTI*(PT(JL)-CST%XTT)+LIMAP%XBCRIAUTI)) ! Autoconversion ri threshold
  ELSE
    ZCRIAUTI(JL)=0.
  END IF
END DO
...
IF(HSUBG_AUCV_RI=='NONE') THEN
  ...
  DO JL=1, KSIZE
    IF (PRIT(JL)>ZCRIAUTI(JL) .AND. LDMICRO(JL)) THEN
      ...
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
    ELSE IF (PRIT(JL)>LIMAP%XRTMIN(4) .AND. LDMICRO(JL)) THEN
      PHLI_HCF(JL)=0.
      PHLI_LCF(JL)=1.
      PHLI_HRI(JL)=0.
      PHLI_LRI(JL)=PRIT(JL)
...
ELSEIF(HSUBG_AUCV_RI=='CLFR') THEN
  ...
  DO JL=1, KSIZE
    IF (PCF(JL)>0. .AND. PRIT(JL)>ZCRIAUTI(JL)*PCF(JL) .AND. LDMICRO(JL)) THEN
      ...
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRIT(JL)>LIMAP%XRTMIN(4) .AND. LDMICRO(JL)) THEN
      PHLI_HCF(JL)=0.
      PHLI_LCF(JL)=PCF(JL)
      PHLI_HRI(JL)=0.0
      PHLI_LRI(JL)=PRIT(JL)
