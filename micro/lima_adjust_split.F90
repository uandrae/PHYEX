...
!
!
USE MODI_CONDENSATION, ONLY: CONDENSATION
...
IF ( BUCONF%LBU_ENABLE ) then
  ...
  IF ( BUCONF%LBUDGET_TH ) CALL TBUDGETS(NBUDGET_TH)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = ZRVS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RV ) CALL TBUDGETS(NBUDGET_RV)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_RC ) ZWKBUD(:,:) = ZRCS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RC ) CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
  !Remark: ZRIS is not modified but source term kept for better coherence with lima_adjust and lima_notadjust
  IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = ZRIS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RI ) CALL TBUDGETS(NBUDGET_RI)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_SV ) then
    IF ( LIMAP%NMOM_C.GE.2) ZWKBUD(:,:) = ZCCS(:,:) * PRHODJ(:,:)
    IF ( LIMAP%NMOM_C.GE.2) &
      CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NC)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
    IF ( LIMAP%LSCAV .AND. LIMAP%LAERO_MASS ) ZWKBUD(:,:) = ZMAS(:,:) * PRHODJ(:,:)
    IF ( LIMAP%LSCAV .AND. LIMAP%LAERO_MASS ) &
      CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_SCAVMASS)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
    IF ( LIMAP%NMOM_C.GE.1 ) then
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
        ZWKBUD(:,:) = ZNFS(:,:, IL) * PRHODJ(:,:)
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
        ZWKBUD(:,:) = ZNAS(:,:, IL) * PRHODJ(:,:)
        CALL TBUDGETS(IDX)%PTR%INIT_PHY(D, 'CEDS', ZWKBUD )
      END DO
    END IF
  END IF
...
IF (LIMAP%LADJ) THEN
   ...
   IF (NEBN%LSUBG_COND) THEN
      PSRCS(D%NIJB:D%NIJE,:)=Z_SRCS(D%NIJB:D%NIJE,:)
   ENDIF
   IF (NEBN%LSUBG_COND .AND. LIMAP%NMOM_C.GE.2 .AND. LIMAP%LACTI) THEN
      ZW_MF=0.
      ZWTOT=PW_NU+ZW_MF
      ZRV2(D%NIJB:D%NIJE,:)=ZRVT(D%NIJB:D%NIJE,:)
      ZRC2(D%NIJB:D%NIJE,:)=ZRCT(D%NIJB:D%NIJE,:)
      CALL LIMA_CCN_ACTIVATION (LIMAP, LIMAW, TNSV, D, CST, NEBN,&
           KCARB, KSOA, KSP, ODUST, OSALT, OORILAM,              &
           PRHODREF, PEXNREF, PPABST, ZT2, PDTHRAD, ZWTOT, &
   ...
   DO II=D%NIJB, D%NIJE
      DO IK=1,SIZE(ZRCS,2)
         IF (ZRCS(II,IK).GE.LIMAP%XRTMIN(2) .AND. ZCCS(II,IK).GE.LIMAP%XCTMIN(2)) THEN
            ...
            ZW(II,IK) = EXP( CST%XALPW - CST%XBETAW/ZT(II,IK) - CST%XGAMW*LOG(ZT(II,IK) ) ) ! es_w
...
IF ( BUCONF%LBU_ENABLE ) THEN
  IF ( BUCONF%LBUDGET_TH ) ZWKBUD(:,:) = PTHS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_TH ) CALL TBUDGETS(NBUDGET_TH)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_RV ) ZWKBUD(:,:) = ZRVS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RV ) CALL TBUDGETS(NBUDGET_RV)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_RC ) ZWKBUD(:,:) = ZRCS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RC ) CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_RI ) ZWKBUD(:,:) = ZRIS(:,:) * PRHODJ(:,:)
  IF ( BUCONF%LBUDGET_RI ) CALL TBUDGETS(NBUDGET_RI)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
  IF ( BUCONF%LBUDGET_SV ) THEN
    IF ( LIMAP%NMOM_C.GE.2) ZWKBUD(:,:) = ZCCS(:,:) * PRHODJ(:,:)
    IF ( LIMAP%NMOM_C.GE.2) &
      CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_NC)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
    IF ( LIMAP%LSCAV .AND. LIMAP%LAERO_MASS ) ZWKBUD(:,:) = ZMAS(:,:) * PRHODJ(:,:)
    IF ( LIMAP%LSCAV .AND. LIMAP%LAERO_MASS ) &
      CALL TBUDGETS(NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_SCAVMASS)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
    IF ( LIMAP%NMOM_C.GE.1 ) THEN
      DO IL = 1, LIMAP%NMOD_CCN
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_FREE - 1 + IL
        ZWKBUD(:,:) = ZNFS(:,:, IL) * PRHODJ(:,:)
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
        IDX = NBUDGET_SV1 - 1 + TNSV%NSV_LIMA_CCN_ACTI - 1 + IL
        ZWKBUD(:,:) = ZNAS(:,:, IL) * PRHODJ(:,:)
        CALL TBUDGETS(IDX)%PTR%END_PHY(D, 'CEDS', ZWKBUD )
      END DO
