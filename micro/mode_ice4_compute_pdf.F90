...
IF(HSUBG_AUCV_RC=='NONE') THEN
 ...
 DO JL=1, KSIZE
    IF (.NOT. LDMICRO(JL)) THEN
      ...
      PHLC_HRC(JL)=0.
      PHLC_LRC(JL)=0.
    ELSE IF (PRCT(JL)>ZRCRAUTC(JL) .AND. LDMICRO(JL)) THEN
      PHLC_HCF(JL)=1.
      PHLC_LCF(JL)=0.
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
...
ELSEIF(HSUBG_AUCV_RC=='CLFR') THEN
 ...
 DO JL=1, KSIZE
    IF (.NOT. LDMICRO(JL)) THEN
      ...
      PHLC_HRC(JL)=0.
      PHLC_LRC(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRCT(JL)>ZRCRAUTC(JL)*PCF(JL) .AND. LDMICRO(JL)) THEN
      PHLC_HCF(JL)=PCF(JL)
      PHLC_LCF(JL)=0.
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
  ...
  IF(HSUBG_PR_PDF=='SIGM') THEN
    ...
    DO JL=1, KSIZE
      IF (.NOT. LDMICRO(JL)) THEN
        ...
        PHLC_HRC(JL)=0.
        PHLC_LRC(JL)=0.
      ELSE IF (PRCT(JL)>ZRCRAUTC(JL)+PSIGMA_RC(JL) .AND. LDMICRO(JL)) THEN
        PHLC_HCF(JL)=1.
        PHLC_LCF(JL)=0.
        PHLC_HRC(JL)=PRCT(JL)
        PHLC_LRC(JL)=0.
  ...
  ELSEIF(HSUBG_PR_PDF=='HLCRECTPDF' .OR. HSUBG_PR_PDF=='HLCISOTRIPDF' .OR. &
    ...
    DO JL=1, KSIZE
      IF (.NOT. LDMICRO(JL)) THEN
        ! Needed to prevent evaluation, in AROME, of the next elseif (after mnh_expand transformation) condition
        ZHLC_RCMAX(JL)=0.
      ELSE IF (PRCT(JL).GT.0. .AND. PCF(JL).GT.0. .AND. LDMICRO(JL)) THEN
    ...
    ELSE IF(HSUBG_PR_PDF=='HLCISOTRIPDF') THEN
      DO JL=1, KSIZE
        IF (.NOT. LDMICRO(JL)) THEN
          ...
          ZHLC_LRCLOCAL(JL)=0.
          ZHLC_HRCLOCAL(JL)=0
        ELSE IF (PRCT(JL).LE.ZRCRAUTC(JL)*PCF(JL) .AND. &
...
IF(HSUBG_AUCV_RI=='NONE') THEN
  ...
  DO JL=1, KSIZE
    IF (.NOT. LDMICRO(JL)) THEN
      ...
      PHLI_HRI(JL)=0.
      PHLI_LRI(JL)=0.
    ELSE IF (PRIT(JL)>ZCRIAUTI(JL) .AND. LDMICRO(JL)) THEN
      PHLI_HCF(JL)=1.
      PHLI_LCF(JL)=0.
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
...
ELSEIF(HSUBG_AUCV_RI=='CLFR') THEN
  ...
  DO JL=1, KSIZE
    IF (.NOT. LDMICRO(JL)) THEN
      ...
      PHLI_HRI(JL)=0.
      PHLI_LRI(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRIT(JL)>ZCRIAUTI(JL)*PCF(JL) .AND. LDMICRO(JL)) THEN
      PHLI_HCF(JL)=PCF(JL)
      PHLI_LCF(JL)=0.
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
