...
IF(BUCONF%LBU_ENABLE) THEN
  ...
  IF (BUCONF%LBUDGET_TH) CALL TBUDGETS(NBUDGET_TH)%PTR%INIT_PHY(D, 'HENU', ZWKBUD)
  IF (BUCONF%LBUDGET_RV) ZWKBUD(:,:) = PRVS(:,:)*PRHODJ(:,:)
  IF (BUCONF%LBUDGET_RV) CALL TBUDGETS(NBUDGET_RV)%PTR%INIT_PHY(D, 'HENU', ZWKBUD)
...
IF(BUCONF%LBU_ENABLE) THEN
  IF (BUCONF%LBUDGET_TH) ZWKBUD(:,:) = PTHS(:,:)*PRHODJ(:,:)
  IF (BUCONF%LBUDGET_TH) CALL TBUDGETS(NBUDGET_TH)%PTR%END_PHY(D, 'HENU', ZWKBUD)
  IF (BUCONF%LBUDGET_RV) ZWKBUD(:,:) = PRVS(:,:)*PRHODJ(:,:)
  IF (BUCONF%LBUDGET_RV) CALL TBUDGETS(NBUDGET_RV)%PTR%END_PHY(D, 'HENU', ZWKBUD)
  IF (BUCONF%LBUDGET_RI) ZWKBUD(:,:) = PRIS(:,:)*PRHODJ(:,:)
  IF (BUCONF%LBUDGET_RI) CALL TBUDGETS(NBUDGET_RI)%PTR%END_PHY(D, 'HENU', ZWKBUD)
ENDIF
...
IF (HSEDIM == 'STAT') THEN
  IF(BUCONF%LBU_ENABLE) THEN
    IF (BUCONF%LBUDGET_RC .AND. OSEDIC) ZWKBUD(:,:) = PRCS(:,:)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RC .AND. OSEDIC) CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = PRRS(:,:)*PRHODJ(:,:)
    ...
    IF (BUCONF%LBUDGET_RS) ZWKBUD(:,:) = PRSS(:,:)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RS) CALL TBUDGETS(NBUDGET_RS)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
    IF (BUCONF%LBUDGET_RG) ZWKBUD(:,:) = PRGS(:,:)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RG) CALL TBUDGETS(NBUDGET_RG)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
    IF (KRR == 7 .AND. BUCONF%LBUDGET_RH) ZWKBUD(:,:) = PRHS(:,:)*PRHODJ(:,:)
    IF (KRR == 7 .AND. BUCONF%LBUDGET_RH) CALL TBUDGETS(NBUDGET_RH)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  ENDIF
  ...
  IF(BUCONF%LBU_ENABLE) THEN
    IF (BUCONF%LBUDGET_RC .AND. OSEDIC) ZWKBUD(:,:) = PRCS(:,:)*PRHODJ(:,:)
    IF (BUCONF%LBUDGET_RC .AND. OSEDIC) CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
    IF (BUCONF%LBUDGET_RR) ZWKBUD(:,:) = PRRS(:,:)*PRHODJ(:,:)
    ...
    IF (KRR == 7 .AND. BUCONF%LBUDGET_RH) ZWKBUD(:,:) = PRHS(:,:)*PRHODJ(:,:)
    IF (KRR == 7 .AND. BUCONF%LBUDGET_RH) CALL TBUDGETS(NBUDGET_RH)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  ENDIF
...
ELSE
  CTEMP='NO SEDIMENTATION SCHEME FOR HSEDIM='//HSEDIM
  CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'RAIN_ICE_OLD', CTEMP)
END IF
