...
USE MODD_BUDGET,         ONLY: TBUDGETDATA_PTR, TBUDGETCONF_t, NBUDGET_RC, NBUDGET_SV1, &
...
USE MODD_FIELDS_ADDRESS, ONLY: IRC, IRG, IRH, IRI, IRR, IRS
!
...
REAL, DIMENSION(D%NIJT,D%NKT),               INTENT(IN)    :: PCONC3D ! Cloud droplet concentration at t
...
!
!
IF (BUCONF%LBUDGET_RC .AND. PARAMI%LSEDIC) ZWKBUD(:, :) = PRS(:, :, IRC) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RC .AND. PARAMI%LSEDIC) CALL TBUDGETS(NBUDGET_RC)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RR) ZWKBUD(:, :) = PRS(:, :, IRR) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RI) ZWKBUD(:, :) = PRS(:, :, IRI) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RI) CALL TBUDGETS(NBUDGET_RI)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RS) ZWKBUD(:, :) = PRS(:, :, IRS) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RS) CALL TBUDGETS(NBUDGET_RS)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RG) ZWKBUD(:, :) = PRS(:, :, IRG) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RG) CALL TBUDGETS(NBUDGET_RG)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RH .AND. KRR==7) ZWKBUD(:, :) = PRS(:, :, IRH) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RH .AND. KRR==7) CALL TBUDGETS(NBUDGET_RH)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
!
! budget of electric charges
IF (BUCONF%LBUDGET_SV .AND. OELEC) THEN
  IF (PARAMI%LSEDIC) ZWKBUD(:, :) = PQCS(:, :) * PRHODJ(:, :)
  IF (PARAMI%LSEDIC) CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+1)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQRS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+2)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQIS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+3)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQSS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+4)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQGS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+5)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
  IF (KRR == 7) ZWKBUD(:, :) = PQHS(:, :) * PRHODJ(:, :)
  IF (KRR == 7) CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+6)%PTR%INIT_PHY(D, 'SEDI', ZWKBUD)
END IF
!
...
ELSE
  CTEMP='no sedimentation scheme for PARAMI%CSEDIM='//PARAMI%CSEDIM
  CALL PRINT_MSG(NVERB_FATAL, 'GEN', 'RAIN_ICE', CTEMP)
END IF
!
!
IF (BUCONF%LBUDGET_RC .AND. PARAMI%LSEDIC) ZWKBUD(:, :) = PRS(:, :, IRC) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RC .AND. PARAMI%LSEDIC) CALL TBUDGETS(NBUDGET_RC)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RR) ZWKBUD(:, :) = PRS(:, :, IRR) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RR) CALL TBUDGETS(NBUDGET_RR)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RI) ZWKBUD(:, :) = PRS(:, :, IRI) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RI) CALL TBUDGETS(NBUDGET_RI)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RS) ZWKBUD(:, :) = PRS(:, :, IRS) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RS) CALL TBUDGETS(NBUDGET_RS)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RG) ZWKBUD(:, :) = PRS(:, :, IRG) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RG) CALL TBUDGETS(NBUDGET_RG)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
IF (BUCONF%LBUDGET_RH .AND. KRR==7) ZWKBUD(:, :) = PRS(:, :, IRH) * PRHODJ(:, :)
IF (BUCONF%LBUDGET_RH .AND. KRR==7) CALL TBUDGETS(NBUDGET_RH)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
!
! Budget for electric charges
IF (BUCONF%LBUDGET_SV .AND. OELEC) THEN
  IF (PARAMI%LSEDIC) ZWKBUD(:, :) = PQCS(:, :) * PRHODJ(:, :)
  IF (PARAMI%LSEDIC) CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+1)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQRS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+2)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQIS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+3)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQSS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+4)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  ZWKBUD(:, :) = PQGS(:, :) * PRHODJ(:, :)
  CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+5)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
  IF (KRR == 7) ZWKBUD(:, :) = PQHS(:, :) * PRHODJ(:, :)
  IF (KRR == 7) CALL TBUDGETS(NBUDGET_SV1-1+NSV_ELECBEG+6)%PTR%END_PHY(D, 'SEDI', ZWKBUD)
END IF
!
