...
REAL, DIMENSION(MERGE(D%NIJT,0,OFLYER),MERGE(D%NKT,0,OFLYER),MERGE(KSV,0,OFLYER)) :: ZWORKWSV
INTEGER :: ISV
!
!*      1.PRELIMINARIES
!         -------------
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZCP(JIJ, JK)=CST%XCPD
    !
    IF (KRR > 0) ZCP(JIJ, JK) = ZCP(JIJ, JK) + CST%XCPV * PRT(JIJ, JK, 1)
  END DO
END DO
...
DO JIJ=IIJB, IIJE
  ZUSLOPE(JIJ)=ZUSLOPE(JIJ)-PSEA_UCU(JIJ)
  ZVSLOPE(JIJ)=ZVSLOPE(JIJ)-PSEA_VCU(JIJ)
END DO
...
IF (TURBN%CTOM=='TM06') THEN
  CALL TM06(D,TURBN,CST,PTHVREF,PBL_DEPTH,PZZ,PSFTH,ZMWTH,ZMTH2)
  !
...
IF(TURBN%LDYNMF) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZWORK1(JIJ, JK)=ZWORK2(JIJ, JK)*PFLXZUMF(JIJ, JK)
    END DO
  END DO
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PDP(JIJ, JK)=PDP(JIJ, JK)-ZWORK2(JIJ, JK)
    END DO
  END DO
...
!
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZWORK1(JIJ, JK)=ZWORK2(JIJ, JK)*PFLXZVMF(JIJ, JK)
    END DO
  END DO
  ...
  IF(PRESENT(PDPMF)) THEN
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PDPMF(JIJ, JK)=PDPMF(JIJ, JK)-ZWORK2(JIJ, JK)
      END DO
    END DO
