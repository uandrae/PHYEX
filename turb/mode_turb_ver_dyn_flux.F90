...
USE MODE_SHUMAN_PHY, ONLY:DXF_PHY
USE MODE_SHUMAN_PHY, ONLY:DXM2D_PHY
USE MODE_SHUMAN_PHY, ONLY:DYF_PHY
USE MODE_SHUMAN_PHY, ONLY:DYM2D_PHY
USE MODE_SHUMAN_PHY, ONLY:DZM_PHY
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK3_2D(JIJ, JK) = TURBN%XIMPL*ZRES(JIJ, JK) + PEXPL*PUM(JIJ, JK)
  END DO
END DO

!
CALL GZ_U_UW_PHY(D, ZSHUGRADWK3_2D,PDZZ, ZGZ_U_UW2D_WORK1)
...
DO JIJ=IIJB, IIJE
  ZSHUGRADWK1_1D(JIJ) = ZFLXZ(JIJ, IKE-IKL) * (TURBN%XIMPL*(ZRES(JIJ, IKE)-ZRES(JIJ, IKE-IKL)) + &
                                          PEXPL*(PUM(JIJ, IKE)-PUM(JIJ, IKE-IKL)))/ ZMXM1D_WORK1(JIJ)
END DO
...
DO JIJ=IIJB, IIJE
  ZSHUGRADWK1_1D(JIJ) = ZFLXZ(JIJ, IKB+IKL) * (TURBN%XIMPL*(ZRES(JIJ, IKB+IKL)-ZRES(JIJ, IKB)) + &
                                          PEXPL*(PUM(JIJ, IKB+IKL)-PUM(JIJ, IKB)))/ ZMXM1D_WORK1(JIJ)
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_WU )
  CALL GZ_U_UW_PHY(D, PUM,PDZZ, ZGZ_U_UW2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK2_2D(JIJ, JK) = ZGZ_U_UW2D_WORK1(JIJ, JK) *ZFLXZ(JIJ, JK)
  END DO
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaW )
    CALL GX_M_U_PHY(D, OFLAT, PTHLM,PDXX,PDZZ,PDZX, ZGX_M_U2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = ZGX_M_U2D_WORK1(JIJ, JK)* ZMZF2D_WORK1(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = ZMXF2D_WORK1(JIJ, JK)    
  END DO
END DO

!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaW )
    IF (KRR>=1) THEN
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaW )
    END IF
    DO JSV=1,KSV
      CALL GX_U_M_PHY(D, OFLAT, PSVM(:,:,JSV),PDXX,PDZZ,PDZX, ZGX_U_M2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Sv_SBG_UaW(:,:,:,JSV) )
    END DO
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK3_2D(JIJ, JK) = TURBN%XIMPL*ZRES(JIJ, JK) + PEXPL*PVM(JIJ, JK)
  END DO
END DO

!
CALL GZ_V_VW_PHY(D, ZSHUGRADWK3_2D,PDZZ, ZGZ_V_VW2D_WORK1)
...
DO JIJ=IIJB, IIJE
  ZSHUGRADWK1_1D(JIJ) = ZFLXZ(JIJ, IKE-IKL) * (TURBN%XIMPL*(ZRES(JIJ, IKE)-ZRES(JIJ, IKE-IKL)) + &
                                         PEXPL*(PVM(JIJ, IKE)-PVM(JIJ, IKE-IKL)))/ ZMYM1D_WORK1(JIJ)
END DO
...
DO JIJ=IIJB, IIJE
  ZSHUGRADWK1_1D(JIJ) = ZFLXZ(JIJ, IKB+IKL) * (TURBN%XIMPL*(ZRES(JIJ, IKB+IKL)-ZRES(JIJ, IKB)) + &
                                         PEXPL*(PVM(JIJ, IKB+IKL)-PVM(JIJ, IKB)))/ ZMYM1D_WORK1(JIJ)
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_WV )
  CALL GZ_V_VW_PHY(D, PVM,PDZZ, ZGZ_V_VW2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK2_2D(JIJ, JK) = ZGZ_V_VW2D_WORK1(JIJ, JK)*ZFLXZ(JIJ, JK)
  END DO
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_V_SBG_UaV )
  CALL SECOND_MNH(ZTIME2)
  TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
END IF
!
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaW , .TRUE. )
    CALL GY_M_V_PHY(D, OFLAT, PTHLM,PDYY,PDZZ,PDZY, ZGY_M_V2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = ZGY_M_V2D_WORK1(JIJ, JK)*ZMZF2D_WORK1(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = ZMYF2D_WORK1(JIJ, JK)    
  END DO
END DO

!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaW , .TRUE. )
    IF (KRR>=1) THEN
      CALL GY_V_M_PHY(D, OFLAT, PRM(:,:,1),PDYY,PDZZ,PDZY, ZGY_V_M2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaW , .TRUE. )
    END IF
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
