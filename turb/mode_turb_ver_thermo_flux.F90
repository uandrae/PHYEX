...
USE MODE_PRANDTL, ONLY: D_M3_WR_W2R_O_DDRDZ, D_M3_WR_W2TH_O_DDRDZ, &
                        D_M3_WR_WR2_O_DDRDZ, D_M3_WR_WTH2_O_DDRDZ, D_M3_WR_WTHR_O_DDRDZ, &
                        D_M3_WTH_W2R_O_DDTDZ, D_M3_WTH_W2TH_O_DDTDZ, D_M3_WTH_WR2_O_DDTDZ, &
                        D_M3_WTH_WTH2_O_DDTDZ, D_M3_WTH_WTHR_O_DDTDZ, D_PHI3DTDZ_O_DDTDZ, &
                        D_PSI3DRDZ_O_DDRDZ, M3_WR_W2R, M3_WR_W2TH, M3_WR_WR2, &
                        M3_WR_WTH2, M3_WR_WTHR, M3_WTH_W2R, M3_WTH_W2TH, M3_WTH_WR2, M3_WTH_WTH2, &
                        M3_WTH_WTHR
...
REAL, DIMENSION(MERGE(D%NIJT,0,TURBN%CTOM=='TM06')), INTENT(INOUT)::  PBL_DEPTH    ! BL depth
...
REAL, DIMENSION(D%NIJT,TURBN%ZTURB_S), INTENT(IN)          ::  PTURB_SPP ! SPP variables
!
!
...
REAL, DIMENSION(D%NIJT,D%NKT) ::ZMZM2D_WORK2
...
IF(TURBN%LLEONARD) THEN
  ...
  IF ( KRRL >= 1 ) THEN
    IF ( KRRI >= 1 ) THEN
      DO JK=1, IKT
        DO JIJ=IIJB, IIJE
          ZCLD_THOLD(JIJ, JK) = PRM(JIJ, JK, 2) + PRM(JIJ, JK, 4)
        END DO
      END DO
    ELSE
      DO JK=1, IKT
        DO JIJ=IIJB, IIJE
          ZCLD_THOLD(JIJ, JK) = PRM(JIJ, JK, 2)
        END DO
      END DO
    END IF
  END IF
...
IF (TURBN%LHARAT) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -ZKEFF(JIJ, JK)*ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = -ZKEFF(JIJ, JK)
    END DO
  END DO
...
ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -PTURB_SPP(JIJ,TURBN%ZCED)*PPHI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = -PTURB_SPP(JIJ,TURBN%ZCED)*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
...
! ! Compute the Leonard terms for thl
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF_LEONARD(JIJ, JK)= TURBN%XCOEFHGRADTHL*PDXX(JIJ, JK)*PDYY(JIJ, JK)/12.0*( &
                         ZWORK1(JIJ, JK)*ZWORK2(JIJ, JK) &
                       + ZWORK3(JIJ, JK)*ZWORK4(JIJ, JK))
    END DO
  END DO
...
! d(w'2th')/dz
IF (GFWTH) THEN
  CALL M3_WTH_W2TH(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,ZKEFF,PTKEM,Z3RDMOMENT,PTURB_SPP)
  CALL D_M3_WTH_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,&
   & PD,PBLL_O_E,PETHETA,ZKEFF,PTKEM,ZWORK1,PTURB_SPP)
  !

  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK)= ZF(JIJ, JK) + Z3RDMOMENT(JIJ, JK) * PFWTH(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = ZDFDDTDZ(JIJ, JK) + ZWORK1(JIJ, JK) &
                                          * PFWTH(JIJ, JK)
    END DO
  END DO
...
! d(w'th'2)/dz
IF (GFTH2) THEN
  CALL M3_WTH_WTH2(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,Z3RDMOMENT,PTURB_SPP)
  CALL D_M3_WTH_WTH2_O_DDTDZ(D,CSTURB,TURBN,Z3RDMOMENT,PREDTH1,PREDR1,&
    & PD,PBLL_O_E,PETHETA,ZWORK1,PTURB_SPP)
  CALL MZM_PHY(D,PFTH2,ZWORK2)
  !

  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = ZF(JIJ, JK) + Z3RDMOMENT(JIJ, JK) &
                                    * ZWORK2(JIJ, JK)
      ZDFDDTDZ(JIJ, JK) = ZDFDDTDZ(JIJ, JK) + ZWORK1(JIJ, JK) &
                                          * ZWORK2(JIJ, JK)
    END DO
...
! d(w'2r')/dz
IF (GFWR) THEN
  CALL M3_WTH_W2R(D,CSTURB,TURBN,PD,ZKEFF,PTKEM,PBLL_O_E,PEMOIST,PDTH_DZ,ZWORK1,PTURB_SPP)
  CALL D_M3_WTH_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,ZKEFF,PTKEM,PBLL_O_E,PEMOIST,ZWORK2,PTURB_SPP)
  !

  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = ZF(JIJ, JK) + ZWORK1(JIJ, JK) * PFWR(JIJ, JK)
...
! d(w'r'2)/dz
IF (GFR2) THEN
  CALL M3_WTH_WR2(D,CSTURB,TURBN,PD,ZKEFF,PTKEM,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PEMOIST,PDTH_DZ,ZWORK1,PTURB_SPP)
  CALL MZM_PHY(D,PFR2,ZWORK2)
  CALL D_M3_WTH_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,&
    & ZKEFF,PTKEM,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PEMOIST,ZWORK3,PTURB_SPP)
  !

  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = ZF(JIJ, JK) + ZWORK1(JIJ, JK) * ZWORK2(JIJ, JK)
...
IF (GFTHR) THEN
  CALL M3_WTH_WTHR(D,CSTURB,TURBN,PREDR1,PD,ZKEFF,PTKEM,PSQRT_TKE,PBETA,&
    & PLEPS,PEMOIST,Z3RDMOMENT,PTURB_SPP)
  CALL D_M3_WTH_WTHR_O_DDTDZ(D,CSTURB,TURBN,Z3RDMOMENT,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,ZWORK1,PTURB_SPP)
...
IF (OOCEAN) THEN    ! ocean model in coupled case
  ...
  DO JIJ=IIJB, IIJE
    ZF(JIJ, IKE+1) =  PSFTHM(JIJ) &
                  *0.5* ( 1. + PRHODJ(JIJ, IKU)/PRHODJ(JIJ, IKE) )
  END DO
...
ELSE ! atmosp bottom
  ...
  IF (TURBN%CTURBDIM=='3DIM') THEN
    ...
    DO JIJ=IIJB, IIJE
      ZF(JIJ, IKB) = ( TURBN%XIMPL*PSFTHP(JIJ) + PEXPL*PSFTHM(JIJ) )   &
                         * PDIRCOSZW(JIJ)                       &
                         * 0.5 * (1. + PRHODJ(JIJ, IKA) / PRHODJ(JIJ, IKB))
    END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZRWTHL(JIJ, JK)= PRHODJ(JIJ, JK)*(PTHLP(JIJ, JK)-PTHLM(JIJ, JK))& 
                                     /PTSTEP
  END DO
END DO
...
IF (TURBN%LLEONARD) THEN
 DO JK=1,IKT
   ...
   DO JIJ=IIJB, IIJE
     ZALT(JIJ, JK) = PZZ(JIJ, JK)-PZS(JIJ) 
   END DO
 ...
 DO JK=1, IKT
   DO JIJ=IIJB, IIJE
     ZWORK2(JIJ, JK) = ZWORK1(JIJ, JK)*ZF_LEONARD(JIJ, JK)
   END DO
 END DO
 ...
 DO JK=1, IKT
   DO JIJ=IIJB, IIJE
     IF ( (ZCLD_THOLD(JIJ, JK) >= TURBN%XCLDTHOLD) .AND. ( ZALT(JIJ, JK) >= TURBN%XALTHGRAD) ) THEN
      ZRWTHL(JIJ, JK) = -ZWORK3(JIJ, JK)
      PTHLP(JIJ, JK)=PTHLM(JIJ, JK)+PTSTEP*ZRWTHL(JIJ, JK)/PRHODJ(JIJ, JK)
     END IF
   END DO
 END DO
END IF
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWORK1(JIJ, JK) = PTHLP(JIJ, JK) - PTHLM(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PRTHLS(JIJ, JK)= PRTHLS(JIJ, JK)  + ZRWTHL(JIJ, JK)
    !
...
IF (TURBN%LLEONARD) THEN
 DO JK=1, IKT
   DO JIJ=IIJB, IIJE
     IF ( (ZCLD_THOLD(JIJ, JK) >= TURBN%XCLDTHOLD) .AND. ( ZALT(JIJ, JK) >= TURBN%XALTHGRAD) ) THEN
      ZFLXZ(JIJ, JK) = ZF_LEONARD(JIJ, JK)
     END IF
   END DO
 END DO
END IF
...
IF ( OFLYER ) THEN
  ...
  DO JK = IKTB + 1, IKTE - 1
    DO JIJ=IIJB, IIJE
      PWTH(JIJ, JK)=0.5*(ZFLXZ(JIJ, JK)+ZFLXZ(JIJ, JK+IKL))
    END DO
  END DO
  !
  DO JIJ=IIJB, IIJE
    PWTH(JIJ, IKB)=0.5*(ZFLXZ(JIJ, IKB)+ZFLXZ(JIJ, IKB+IKL)) 
  END DO
  !
  IF (OOCEAN) THEN
    DO JIJ=IIJB, IIJE
      PWTH(JIJ, IKE)=0.5*(ZFLXZ(JIJ, IKE)+ZFLXZ(JIJ, IKE+IKL))
      PWTH(JIJ, IKA)=0.
      PWTH(JIJ, IKU)=PWTH(JIJ, IKE)! not used
    END DO
  ELSE
    DO JIJ=IIJB, IIJE
      PWTH(JIJ, IKA)=0.5*(ZFLXZ(JIJ, IKA)+ZFLXZ(JIJ, IKA+IKL))
      PWTH(JIJ, IKE)=PWTH(JIJ, IKE-IKL)
      PWTH(JIJ, IKU)=0.
    END DO
  END IF
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PTP(JIJ, JK)= CST%XG*CST%XALPHAOC * ZMZF2D_WORK1(JIJ, JK)
  END DO
END DO
...
ELSE
  IF (KRR /= 0) THEN
    CALL MZM_PHY(D, PETHETA, ZMZM2D_WORK1)
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PTP(JIJ, JK)  =  PBETA(JIJ, JK) * ZMZF2D_WORK1(JIJ, JK)
  END DO
END DO
...
!
    ...
    DO JIJ=IIJB, IIJE
      PTP(JIJ, IKB)=  PBETA(JIJ, IKB) * PETHETA(JIJ, IKB) *   &
                     0.5 * ( ZFLXZ(JIJ, IKB) + ZFLXZ(JIJ, IKB+IKL) )
    END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PWTHV(JIJ, JK) = ZMZM2D_WORK1(JIJ, JK) * ZFLXZ(JIJ, JK)
  END DO
END DO
...
DO JIJ=IIJB, IIJE
  PWTHV(JIJ, IKB) = PETHETA(JIJ, IKB) * ZFLXZ(JIJ, IKB)
END DO
...
IF (OOCEAN) THEN
  ...
  DO JIJ=IIJB, IIJE
    PWTHV(JIJ, IKE) = PETHETA(JIJ, IKE) * ZFLXZ(JIJ, IKE)
  END DO
...
IF(TURBN%LPROJQITURB) THEN
 IF ( KRRL >= 1 ) THEN
   ...
   DO JK=1, IKT
     DO JIJ=IIJB, IIJE
       ZWORK1(JIJ, JK) = ZFLXZ(JIJ, JK)/PDZZ(JIJ, JK)
     END DO
   END DO
   ...
   IF ( KRRI >= 1 ) THEN
     ...
     DO JK=1, IKT
       DO JIJ=IIJB, IIJE
         PRRS(JIJ, JK, 2) = PRRS(JIJ, JK, 2) -                                        &
                         PRHODJ(JIJ, JK)*PATHETA(JIJ, JK)*2.*PSRCM(JIJ, JK)& 
                         *ZWORK2(JIJ, JK) *(1.0-PFRAC_ICE(JIJ, JK))
         PRRS(JIJ, JK, 4) = PRRS(JIJ, JK, 4) -                                        &
                         PRHODJ(JIJ, JK)*PATHETA(JIJ, JK)*2.*PSRCM(JIJ, JK)&
                         * ZWORK2(JIJ, JK)*PFRAC_ICE(JIJ, JK)
       END DO
     END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = ZMZF2D_WORK1(JIJ, JK)  
  END DO
END DO

!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_WThl )
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_W_SBG_WThl )
  CALL GZ_W_M_PHY(D, PWM,PDZZ, ZGZ_W_M2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = ZGZ_W_M2D_WORK1(JIJ, JK)*ZMZF2D_WORK1(JIJ, JK)  
  END DO
END DO

!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaThl )
  
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = PDTH_DZ(JIJ, JK)*ZFLXZ(JIJ, JK)
  END DO
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaThl )
  CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZWKLES(JIJ, JK) = -PTURB_SPP(JIJ,TURBN%ZCTP)*PSQRT_TKE(JIJ, JK)/PLM(JIJ, JK)*ZMZF2D_WORK1(JIJ, JK)  
  END DO
END DO

!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_ThlPz )
  CALL MZM_PHY(D, PETHETA, ZMZM2D_WORK1)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_WThv, .TRUE. )
  IF (KRR>=1) THEN
    
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = PDR_DZ(JIJ, JK)*ZFLXZ(JIJ, JK)
  END DO
END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaThl )
  END IF
  !
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK) = - ZFLXZ(JIJ, JK) / ZA(JIJ, JK) * PDZZ(JIJ, JK)
    END DO
  END DO
  DO JIJ=IIJB, IIJE
    ZA(JIJ, IKB) = PTURB_SPP(JIJ,TURBN%ZCED)*PPHI3(JIJ, IKB)*ZKEFF(JIJ, IKB)
  END DO

  CALL MZF_PHY(D, ZA, ZMZF2D_WORK1)
...
!
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK) = MIN(MAX(ZA(JIJ, JK),-1000.),1000.)
    END DO
  END DO
...
!
IF (TURBN%CTOM=='TM06') CALL TM06_H(D,TURBN,PTSTEP,PZZ,ZFLXZ,PBL_DEPTH)
!
!----------------------------------------------------------------------------
!
!
...
IF (KRR /= 0) THEN
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZF(JIJ, JK) = -PTURB_SPP(JIJ,TURBN%ZCED)*PPSI3(JIJ, JK)*ZKEFF(JIJ, JK)& 
                                    *ZWORK1(JIJ, JK)/PDZZ(JIJ, JK)
      ZDFDDRDZ(JIJ, JK) = -PTURB_SPP(JIJ,TURBN%ZCED)*ZKEFF(JIJ, JK)*ZWORK2(JIJ, JK)
    END DO
  END DO
 ...
 END IF
  ...
  ! d(w'2r')/dz
  IF (GFWR) THEN
    CALL M3_WR_W2R(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,ZKEFF,PTKEM,Z3RDMOMENT,PTURB_SPP)
    CALL D_M3_WR_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,&
     & PBLL_O_E,PEMOIST,ZKEFF,PTKEM,ZWORK1,PTURB_SPP)
    !

    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZF(JIJ, JK)= ZF(JIJ, JK) + Z3RDMOMENT(JIJ, JK) * PFWR(JIJ, JK)
        ZDFDDRDZ(JIJ, JK) = ZDFDDRDZ(JIJ, JK) + ZWORK1(JIJ, JK) &
                                            * PFWR(JIJ, JK)
      END DO
    END DO
  ...
  ! d(w'r'2)/dz
  IF (GFR2) THEN
    CALL M3_WR_WR2(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,Z3RDMOMENT,PTURB_SPP)
    CALL MZM_PHY(D,PFR2,ZWORK1)
    CALL D_M3_WR_WR2_O_DDRDZ(D,CSTURB,TURBN,Z3RDMOMENT,PREDR1,&
     & PREDTH1,PD,PBLL_O_E,PEMOIST,ZWORK2,PTURB_SPP)
    !
  ...
  IF (GFWTH) THEN
    CALL M3_WR_W2TH(D,CSTURB,TURBN,PD,ZKEFF,&
     & PTKEM,PBLL_O_E,PETHETA,PDR_DZ,ZWORK1,PTURB_SPP)
    CALL D_M3_WR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,&
     & PD,ZKEFF,PTKEM,PBLL_O_E,PETHETA,ZWORK2,PTURB_SPP)
    !

    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZF(JIJ, JK) = ZF(JIJ, JK) + ZWORK1(JIJ, JK) * PFWTH(JIJ, JK)
        ZDFDDRDZ(JIJ, JK) = ZDFDDRDZ(JIJ, JK) + ZWORK2(JIJ, JK) &
                                            * PFWTH(JIJ, JK)
      END DO
    END DO
  ...
  IF (GFTH2) THEN
    ...
    & PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,PDR_DZ,ZWORK2,PTURB_SPP)
    CALL D_M3_WR_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,&
     &ZKEFF,PTKEM,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,ZWORK3,PTURB_SPP)
    !
  ...
  IF (GFTHR) THEN
    CALL M3_WR_WTHR(D,CSTURB,TURBN,PREDTH1,PD,ZKEFF,PTKEM,PSQRT_TKE,PBETA,&
     & PLEPS,PETHETA,Z3RDMOMENT,PTURB_SPP)
  ...
  !
   !special case at sfc
    ...
    IF (TURBN%CTURBDIM=='3DIM') THEN
      ...
      DO JIJ=IIJB, IIJE
        ZF(JIJ, IKB) = ( TURBN%XIMPL*PSFRP(JIJ) + PEXPL*PSFRM(JIJ) )       &
                             * PDIRCOSZW(JIJ)                       &
                           * 0.5 * (1. + PRHODJ(JIJ, IKA) / PRHODJ(JIJ, IKB))
      END DO
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZRWRNP(JIJ, JK) = PRHODJ(JIJ, JK)*(PRP(JIJ, JK)-PRM(JIJ, JK, 1))& 
                                       /PTSTEP
    END DO
  END DO
  ...
  IF (TURBN%LLEONARD) THEN
   ...
   DO JK=1, IKT
     DO JIJ=IIJB, IIJE
       IF ( (ZCLD_THOLD(JIJ, JK) >= TURBN%XCLDTHOLD ) .AND. ( ZALT(JIJ, JK) >= TURBN%XALTHGRAD ) ) THEN
        ZRWRNP(JIJ, JK) =  -ZWORK3(JIJ, JK)
        PRP(JIJ, JK)=PRM(JIJ, JK, 1)+PTSTEP*ZRWTHL(JIJ, JK)/PRHODJ(JIJ, JK)
       END IF
     END DO
   END DO
  END IF
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PRRS(JIJ, JK, 1) = PRRS(JIJ, JK, 1) + ZRWRNP(JIJ, JK)
      !
      ...
      ZFLXZ(JIJ, JK)   = ZF(JIJ, JK)                                                &
                     + TURBN%XIMPL * ZDFDDRDZ(JIJ, JK) * ZWORK2(JIJ, JK) / PDZZ(JIJ, JK)
    END DO
  END DO
  ...
  IF ( OFLYER ) THEN
    ...
    DO JK=IKTB+1,IKTE-1
      DO JIJ=IIJB, IIJE
        PWRC(JIJ, JK)=0.5*(ZFLXZ(JIJ, JK)+ZFLXZ(JIJ, JK+IKL))
      END DO
    END DO
    DO JIJ=IIJB, IIJE
      PWRC(JIJ, IKB)=0.5*(ZFLXZ(JIJ, IKB)+ZFLXZ(JIJ, IKB+IKL))
    END DO
    !
    IF (OOCEAN) THEN
      DO JIJ=IIJB, IIJE
        PWRC(JIJ, IKE)=0.5*(ZFLXZ(JIJ, IKE)+ZFLXZ(JIJ, IKE+IKL))
        PWRC(JIJ, IKA)=0.
        PWRC(JIJ, IKE+1)=ZFLXZ(JIJ, IKE+1)
      END DO
    ELSE
      DO JIJ=IIJB, IIJE
        PWRC(JIJ, IKA)=0.5*(ZFLXZ(JIJ, IKA)+ZFLXZ(JIJ, IKA+IKL))
        PWRC(JIJ, IKE)=PWRC(JIJ, IKE-IKL)
        PWRC(JIJ, IKU)=0.
      END DO
    END IF
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZA(JIJ, JK)=  -CST%XG * CST%XBETAOC * ZMZF2D_WORK1(JIJ, JK)  
  END DO
END DO

!
ELSE
    CALL MZM_PHY(D, PEMOIST, ZMZM2D_WORK1)
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZA(JIJ, JK)   =  PBETA(JIJ, JK) * ZMZF2D_WORK1(JIJ, JK)
  END DO
END DO
...
!
    ...
    DO JIJ=IIJB, IIJE
      ZA(JIJ, IKB) =  PBETA(JIJ, IKB) * PEMOIST(JIJ, IKB) *   &
                     0.5 * ( ZFLXZ(JIJ, IKB) + ZFLXZ(JIJ, IKB+IKL) )
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PTP(JIJ, JK) = PTP(JIJ, JK) + ZA(JIJ, JK)
      END DO
    END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PWTHV(JIJ, JK) = PWTHV(JIJ, JK) + ZMZM2D_WORK1(JIJ, JK) * ZFLXZ(JIJ, JK)  
  END DO
END DO
...
!
  ...
  DO JIJ=IIJB, IIJE
    PWTHV(JIJ, IKB) = PWTHV(JIJ, IKB) + PEMOIST(JIJ, IKB) * ZFLXZ(JIJ, IKB)
  END DO

  IF (OOCEAN) THEN

    DO JIJ=IIJB, IIJE
      PWTHV(JIJ, IKE) = PWTHV(JIJ, IKE) + PEMOIST(JIJ, IKE)* ZFLXZ(JIJ, IKE)
    END DO
   ...
   IF ( KRRL >= 1 ) THEN
     ...
     IF ( KRRI >= 1 ) THEN
       ...
       DO JK=1, IKT
         DO JIJ=IIJB, IIJE
           PRRS(JIJ, JK, 2) = PRRS(JIJ, JK, 2) -                                        &
                           PRHODJ(JIJ, JK)*PAMOIST(JIJ, JK)*2.*PSRCM(JIJ, JK)& 
                           *ZWORK1(JIJ, JK) *(1.0-PFRAC_ICE(JIJ, JK))
           PRRS(JIJ, JK, 4) = PRRS(JIJ, JK, 4) -                                        &
                           PRHODJ(JIJ, JK)*PAMOIST(JIJ, JK)*2.*PSRCM(JIJ, JK)&
                           *ZWORK1(JIJ, JK) *PFRAC_ICE(JIJ, JK)
         END DO
       END DO
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_WRt )
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_W_SBG_WRt )
    CALL GZ_W_M_PHY(D, PWM,PDZZ, ZGZ_W_M2D_WORK1)
CALL MZF_PHY(D, ZFLXZ, ZMZF2D_WORK1)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_W_SBG_UaRt )
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Thl_SBG_UaRt )
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_RES_ddxa_Rt_SBG_UaRt )
    CALL MZM_PHY(D, PEMOIST, ZMZM2D_WORK1)
...
!
CALL LES_MEAN_SUBGRID_PHY(D, TLES, ZWKLES, TLES%X_LES_SUBGRID_RtPz )
    CALL SECOND_MNH(ZTIME2)
    TLES%XTIME_LES = TLES%XTIME_LES + ZTIME2 - ZTIME1
  END IF
!
...
! yet modify to use length scale and tke at half levels from vdfexcuhl
 ...
 DO JK=1, IKT
   DO JIJ=IIJB, IIJE
     ZWORK1(JIJ, JK) = TURBN%XIMPL * PTHLP(JIJ, JK) + PEXPL * PTHLM(JIJ, JK)
   END DO
 END DO
 ...
 IF (TURBN%LHARAT) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK)   = ZWORK2(JIJ, JK)/ PDZZ(JIJ, JK) * &
                                     (-PLM(JIJ, JK)*PSQRT_TKE(JIJ, JK))
    END DO
  END DO
 ...
 ELSE
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZWORK1(JIJ, JK) = PLM(JIJ, JK)*PSQRT_TKE(JIJ, JK)
    END DO
  END DO
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZA(JIJ, JK)   = ZWORK2(JIJ, JK)/ PDZZ(JIJ, JK) * &
                                      (-PPHI3(JIJ, JK)*ZWORK3(JIJ, JK)) * PTURB_SPP(JIJ,TURBN%ZCED)
    END DO
  END DO
 ...
 ENDIF
  ...
  DO JIJ=IIJB, IIJE
    ZA(JIJ, IKB) = (TURBN%XIMPL*PSFTHP(JIJ) + PEXPL*PSFTHM(JIJ)) * PDIRCOSZW(JIJ)
  END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = PAMOIST(JIJ, JK) * 2.* PSRCM(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = PATHETA(JIJ, JK) * 2.* PSRCM(JIJ, JK)
  END DO
END DO

!
CALL MZM_PHY(D, ZSHUGRADWK1_2D, ZMZM2D_WORK2)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZFLXZ(JIJ, JK) = ZMZM2D_WORK1(JIJ, JK) * ZFLXZ(JIJ, JK) + &
                     ZMZM2D_WORK2(JIJ, JK) * ZA(JIJ, JK)
  END DO
END DO
