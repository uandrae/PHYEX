...
USE MODE_ll, ONLY: CLEANLIST_LL, UPDATE_HALO_LL
USE MODE_ARGSLIST_ll_PHY, ONLY: ADD3DFIELD_ll_PHY
!
USE MODI_GET_HALO, ONLY: GET_HALO_PHY
...
REAL, DIMENSION(D%NIJT,D%NKT) ::ZDZF2D_WORK1
INTEGER :: JIJ,JK
!
!----------------------------------------------------------------------------
!
...
IF (OOCEAN) THEN
  ...
  DO JIJ=IIJB, IIJE
    PDP(JIJ, IKE) = PDP(JIJ, IKE) * (1. + PDZZ(JIJ, IKE)/PDZZ(JIJ, IKE+1))
  END DO
ELSE
  ! W(IKB+1) value stored in PDP(IKB) to the mass localization tke(IKB)
  DO JIJ=IIJB, IIJE
    PDP(JIJ, IKB) = PDP(JIJ, IKB) * (1. + PDZZ(JIJ, IKB+IKL)/PDZZ(JIJ, IKB))
  END DO
END IF
!
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ...
    ZSOURCE(JIJ, JK) = ( PRTKES(JIJ, JK) +  PRTKEMS(JIJ, JK) ) &
       ...
       - PEXPL * ZFLX(JIJ, JK) * PTKEM(JIJ, JK)
  END DO
END DO
!
...
IF (OOCEAN) THEN
  ...
  DO JIJ=IIJB, IIJE
    ZSOURCE(JIJ, IKE)=ZSOURCE(JIJ, IKE)-1.E2*((PSFUM(JIJ)**2 + PSFVM(JIJ)**2)**1.5) /PDZZ(JIJ, IKE)  
  END DO
END IF
...
!
!
CALL MZM_PHY(D, ZKEFF, ZMZM2D_WORK1)
CALL MZM_PHY(D, PRHODJ, ZMZM2D_WORK2)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZA(JIJ, JK) = - PTSTEP * CSTURB%XCET * ZMZM2D_WORK1(JIJ, JK) * ZMZM2D_WORK2(JIJ, JK) / PDZZ(JIJ, JK)**2
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZFLX_PTSTEP(JIJ, JK) = PTSTEP*ZFLX(JIJ, JK)
  END DO
END DO
...
IF (ODIAG_IN_RUN) THEN
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PCURRENT_TKE_DISS(JIJ, JK) = ZFLX(JIJ, JK) &
...
IF(TURBN%LTKEMINTURB) THEN
 DO JK=1, IKT
   DO JIJ=IIJB, IIJE
     GTKENEG(JIJ, JK) =  ZRES(JIJ, JK) <= TURBN%XTKEMIN
     IF ( GTKENEG(JIJ, JK) ) THEN 
       ZRES(JIJ, JK) = TURBN%XTKEMIN
     END IF
   END DO
 END DO
END IF
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PTDISS(JIJ, JK) = - ZFLX(JIJ, JK)*(PEXPL*PTKEM(JIJ, JK) &
                                          + TURBN%XIMPL*ZRES(JIJ, JK))
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = TURBN%XIMPL * ZRES(JIJ, JK) + PEXPL * PTKEM(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZFLX(JIJ, JK)   = - CSTURB%XCET * ZMZM2D_WORK1(JIJ, JK) *   &
                      ZDZM2D_WORK1(JIJ, JK) / PDZZ(JIJ, JK)
  END DO
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSHUGRADWK1_2D(JIJ, JK) = ZMZM2D_WORK1(JIJ, JK) * ZFLX(JIJ, JK) / PDZZ(JIJ, JK)
  END DO
END DO

!
CALL DZF_PHY(D, ZSHUGRADWK1_2D, ZDZF2D_WORK1)

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PTR(JIJ, JK)= PTR(JIJ, JK) - ZDZF2D_WORK1(JIJ, JK) /PRHODJ(JIJ, JK)
  END DO
END DO
...
IF (BUCONF%LBUDGET_TKE) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZMWORK1(JIJ, JK) = PDP(JIJ, JK) * PRHODJ(JIJ, JK)
    END DO
  END DO
  
  CALL TBUDGETS(NBUDGET_TKE)%PTR%ADD_PHY(D, 'DP', ZMWORK1)
  !
  ! Thermal production
  
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZMWORK1(JIJ, JK) = PTP(JIJ, JK) * PRHODJ(JIJ, JK)
    END DO
  END DO
  
  CALL TBUDGETS(NBUDGET_TKE)%PTR%ADD_PHY(D, 'TP', ZMWORK1)
  !
  ! Dissipation
  ...
  CALL TBUDGETS(NBUDGET_TKE)%PTR%ADD_PHY(D, 'DISS',ZMWORK1)
END IF 
!
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ...
    PTDIFF(JIJ, JK) =  ZRES(JIJ, JK) / PTSTEP - PRTKES(JIJ, JK)&
                                         /PRHODJ(JIJ, JK) &
                               & - PDP(JIJ, JK)- PTP(JIJ, JK) - PTDISS(JIJ, JK)
  END DO
END DO

!
IF (BUCONF%LBUDGET_TKE) CALL TBUDGETS(NBUDGET_TKE)%PTR%INIT_PHY(D, 'TR', PRTKES)
!

DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PRTKES(JIJ, JK) = ZRES(JIJ, JK) * PRHODJ(JIJ, JK) / PTSTEP &
                                        -  PRTKEMS(JIJ, JK)
  END DO
END DO
...
!
IF (BUCONF%LBUDGET_TKE) CALL TBUDGETS(NBUDGET_TKE)%PTR%END_PHY(D, 'TR', PRTKES)
!
!----------------------------------------------------------------------------
!
