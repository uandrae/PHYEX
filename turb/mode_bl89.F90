...
CONTAINS
!     ######spl
      SUBROUTINE BL89(D,CST,CSTURB,TURBN,PZZ,PDZZ,PTHVREF,PTHLM,KRR,PRM,PTKEM,PSHEAR,PLM,OOCEAN,PTURB_SPP)
...
REAL, DIMENSION(D%NIJT,TURBN%ZTURB_S),              INTENT(IN)  ::  PTURB_SPP       ! switch for Ocean model version
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZSQRT_TKE(JIJ, JK) = SQRT(PTKEM(JIJ, JK))
  END DO
END DO
...
IF(KRR /= 0) THEN
  ...
  DO JRR=1,KRR
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZSUM(JIJ, JK) = ZSUM(JIJ, JK)+PRM(JIJ, JK, JRR)
      END DO
    END DO
  ENDDO
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZVPT(JIJ, JK)=PTHLM(JIJ, JK) * ( 1. + ZRVORD*PRM(JIJ, JK, 1) )  &
                               / ( 1. + ZSUM(JIJ, JK) )
    END DO
  END DO
ELSE
...
DO JK=IKTB,IKTE
  DO JIJ=IIJB,IIJE
    ...
    ZLWORK2=1.d0 + ZPOTE**PTURB_SPP(JIJ,TURBN%ZBL89EXP)
    PLM(JIJ,JK) = ZLWORK1*(2./ZLWORK2)**PTURB_SPP(JIJ,TURBN%ZUSRBL89)
