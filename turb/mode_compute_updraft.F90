...
IF (OENTR_DETR) THEN
  ...
  IF(PARAMMF%LPZ_EXP_LOG) THEN
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZWK(JIJ, JK)=LOG(PPABSM(JIJ, JK))
      END DO
    END DO
    CALL MZM_MF(D, ZWK, ZPRES_F)
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZPRES_F(JIJ, JK)=EXP(ZPRES_F(JIJ, JK))
      END DO
    END DO
  ELSE
  ...
  DO JIJ=IIJB, IIJE
    ...
    PTHV_UP(JIJ, IKB) = PTH_UP(JIJ, IKB)*&
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ...
      ZG_O_THVREF_UP(JIJ, JK)=CST%XG/PTHV_UP(JIJ, JK)
    END DO
  END DO
  ...
  DO JIJ=IIJB, IIJE
    ZLUPSURF(JIJ)=MAX(ZLUPSURF(JIJ),1.E-10)
  ...
  IF (PARAMMF%LGZ) THEN
    ...
    ENDIF
    DO JIJ=IIJB, IIJE
      ZSURF(JIJ)=TANH(PARAMMF%XGZ*SQRT(PDX*PDY)/ZLUPSURF(JIJ))
    END DO
  ELSE
    ZSURF(IIJB:IIJE)=1.
  ENDIF
...
DO JK=IKB,IKE-IKL,IKL
  ...
  IF(PARAMMF%LRELAX_ALPHA_MF) THEN
    DO JIJ=IIJB, IIJE
      ZRELAX_ALPHA_COEFF(JIJ)=1./(1.-PFRAC_UP(JIJ, JK))
    END DO
  ELSE
  ...
  IF(PARAMMF%LMIXUV) THEN
    IF(JK/=IKB) THEN
      DO JIJ=IIJB, IIJE
        IF (GTEST(JIJ)) THEN
          PU_UP(JIJ, JK+IKL) = (PU_UP(JIJ, JK)*(1-0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) + &
                                          &PUM(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)+ &
                            ...
                            /(1+0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ))
          PV_UP(JIJ, JK+IKL) = (PV_UP(JIJ, JK)*(1-0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) + &
                                          &PVM(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)+ &
                            0.5*PARAMMF%XPRES_UV*(PZZ(JIJ, JK+IKL)-PZZ(JIJ, JK))*&
  ...
  IF(PARAMMF%LMIXTKE) THEN
    DO JIJ=IIJB, IIJE
      IF (GTEST(JIJ)) THEN
        PTKE_UP(JIJ, JK+IKL) = (PTKE_UP(JIJ, JK)*(1-0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) + &
                                     ZTKEM_F(JIJ, JK)*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) &
                                    /(1+0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ))
      END IF
    END DO
  ENDIF
  ...
  DO JSV=1,KSV 
    ...
    DO JIJ=IIJB, IIJE
      IF (GTEST(JIJ)) THEN 
        PSV_UP(JIJ, JK+IKL, JSV) = (PSV_UP(JIJ, JK, JSV)*(1-0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) + &
                                        PSVM(JIJ, JK, JSV)*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ)) &
                                       /(1+0.5*ZRELAX_ALPHA_COEFF(JIJ)*ZMIX2(JIJ))
      END IF
    END DO
  END DO  
...
IF(OENTR_DETR) THEN
  ...
  DO JK=1,IKT
    DO JIJ=IIJB, IIJE
      ...
      IF(PARAMMF%LVERLIMUP) THEN
        ZCOEF(JIJ, JK) = (1.-(KDEPTH(JIJ, JK)-ZDEPTH_MAX1)/(ZDEPTH_MAX2-ZDEPTH_MAX1))
      ELSE
...
DO JIJ=IIJB, IIJE
  IF(KK/=KKB)THEN
    ZCOEFF_MINUS_HALF_THV(JIJ)=((PTHVM(JIJ, KK)-PTHVM(JIJ, KK-KKL))/PDZZ(JIJ, KK))
    ZTHV_MINUS_HALF(JIJ) = PTHVM(JIJ, KK) - &
                                 & ZCOEFF_MINUS_HALF_THV(JIJ)*0.5*(PZZ(JIJ, KK+KKL)-PZZ(JIJ, KK))
  ELSE
    ZCOEFF_MINUS_HALF_THV(JIJ)=0.
    ZTHV_MINUS_HALF(JIJ) = PTHVM(JIJ, KK)
  ENDIF
  ZCOEFF_PLUS_HALF_THV(JIJ)  = ((PTHVM(JIJ, KK+KKL)-PTHVM(JIJ, KK))/PDZZ(JIJ, KK+KKL))
  ZTHV_PLUS_HALF(JIJ)  = PTHVM(JIJ, KK) + &
                               & ZCOEFF_PLUS_HALF_THV(JIJ)*0.5*(PZZ(JIJ, KK+KKL)-PZZ(JIJ, KK))
END DO
