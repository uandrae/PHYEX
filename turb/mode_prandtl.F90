...
!!       PTURB_SPP(JIJ,TURBN%ZCEP),XCPR2    : constants for the turbulent prandtl numbers
...
LOGICAL,                INTENT(IN)   ::  OTURB_DIAG   ! switch to write some
                                 ! diagnostic fields in the syncronous file
...
IF (OOCEAN) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PREDTH1(JIJ, JK)= PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK)*ZWORK1(JIJ, JK)
    END DO
  END DO
...
ELSE
  IF (KRR /= 0) THEN                ! moist case
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PREDTH1(JIJ, JK)= PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK) * PETHETA(JIJ, JK) &
                                          * ZWORK1(JIJ, JK)
        PREDR1(JIJ, JK) = PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK) * PEMOIST(JIJ, JK) &
                                          * ZWORK2(JIJ, JK)
      END DO
    END DO
...
DO JSV=1,KSV
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PREDS1(JIJ, JK, JSV)=PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK)*ZWORK1(JIJ, JK)
    END DO
  END DO
...
!
  ...
  IF (KRR /= 0) THEN                 ! moist 3D case
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRED2TH3(JIJ, JK)= PREDTH1(JIJ, JK)**2+(PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK) &
                                           *PETHETA(JIJ, JK) )**2 * ZWORK2(JIJ, JK)
    !
        PRED2R3(JIJ, JK)= PREDR1(JIJ, JK)**2 + (PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK) &
                                         * PEMOIST(JIJ, JK))**2 * ZWORK3(JIJ, JK)
    !
        PRED2THR3(JIJ, JK)= PREDR1(JIJ, JK) * PREDTH1(JIJ, JK) +  PTURB_SPP(JIJ,TURBN%ZCEP)**2 &
                                            * PBLL_O_E(JIJ, JK)**2   &
...
!
  ELSE                 ! dry 3D case in a 2D model
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRED2TH3(JIJ, JK) = PREDTH1(JIJ, JK)**2 +  PTURB_SPP(JIJ,TURBN%ZCEP)**2 & 
...
!
    ...
    ELSE
      ...
      IF ( JSV == 1 ) THEN
        ...
        DO JK=1, IKT
          DO JIJ=IIJB, IIJE
            ZWORK1(JIJ, JK) = (CST%XG / PTHVREF(JIJ, JK) * PLM(JIJ, JK) * PLEPS(JIJ, JK) / PTKEM(JIJ, JK))**2
          END DO
        END DO
...
IKT=D%NKT
!
GHTURBDIM_3DIM = (HTURBDIM=='3DIM')
!

IF (GHTURBDIM_3DIM) THEN
        !* 3DIM case
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      IF (OUSERV) THEN
...
END SUBROUTINE PHI3
!----------------------------------------------------------------------------
SUBROUTINE PSI_SV(D,CSTURB,TURBN,KSV,PREDTH1,PREDR1,PREDS1,PRED2THS,PRED2RS,PPHI3,PPSI3,PPSI_SV,PTURB_SPP)
  USE MODD_TURB_n,         ONLY: TURB_t
  USE MODD_CTURB,          ONLY: CSTURB_t
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
  TYPE(CSTURB_t),                  INTENT(IN)      :: CSTURB
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PPSI_SV(JIJ, JK, JSV) = ( 1.                                             &
        - (PTURB_SPP(JIJ,TURBN%ZCEP)/2+PTURB_SPP(JIJ,TURBN%ZCEP)/2) * &
        (PRED2THS(JIJ, JK, JSV)/PREDS1(JIJ, JK, JSV)-PREDTH1(JIJ, JK)) &
        - (PTURB_SPP(JIJ,TURBN%ZCEP)/2+PTURB_SPP(JIJ,TURBN%ZCEP)/2) * &     
        (PRED2RS(JIJ, JK, JSV)/PREDS1(JIJ, JK, JSV)-PREDR1(JIJ, JK)) &
        - PTURB_SPP(JIJ,TURBN%ZCEP)/2 * &              
        PREDTH1(JIJ, JK) * PPHI3(JIJ, JK) &
        - PTURB_SPP(JIJ,TURBN%ZCEP)/2 * PREDR1(JIJ, JK) * PPSI3(JIJ, JK)                 &
                    ) / (  1. + PTURB_SPP(JIJ,TURBN%ZCEP)/2 * ( PREDTH1(JIJ, JK) + PREDR1(JIJ, JK) ) )           
...
SUBROUTINE D_PHI3DTDZ2_O_DDTDZ(D,CSTURB,TURBN,PPHI3,PREDTH1,PREDR1,PRED2TH3,PRED2THR3,PDTDZ,HTURBDIM,OUSERV,PD_PHI3DTDZ2_O_DDTDZ)
  ...
  INTEGER :: IKB, IKE, JIJ,JK, IIJB,IIJE,IKT
  REAL, DIMENSION(D%NIJT,D%NKT) :: ZPHI3
!
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
...
CALL SMOOTH_TURB_FUNCT(D,CSTURB,TURBN,PPHI3,ZPHI3,PD_PHI3DTDZ2_O_DDTDZ)
!
!
...
!----------------------------------------------------------------------------
SUBROUTINE M3_WTH_WTH2(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PM3_WTH_WTH2,PTURB_SPP)
  USE MODD_TURB_n,         ONLY: TURB_t
  USE MODD_CTURB,          ONLY: CSTURB_t
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_WTH_WTH2(JIJ, JK) = PTURB_SPP(JIJ,TURBN%ZCED)*PBLL_O_E(JIJ, JK)&
...
!----------------------------------------------------------------------------
SUBROUTINE D_M3_WTH_WTH2_O_DDTDZ(D,CSTURB,TURBN,PM3_WTH_WTH2,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PD_M3_WTH_WTH2_O_DDTDZ,PTURB_SPP)
  USE MODD_TURB_n,         ONLY: TURB_t
  USE MODD_CTURB,          ONLY: CSTURB_t
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ...
    (0.5*PTURB_SPP(JIJ,TURBN%ZCED)*PBLL_O_E(JIJ, JK)*PETHETA(JIJ, JK)*0.5/CSTURB%XCTD/PD(JIJ, JK) &
    - PM3_WTH_WTH2(JIJ, JK)/PD(JIJ, JK)&
    *(1.5+PREDTH1(JIJ, JK)+PREDR1(JIJ, JK))  )&
    * PBLL_O_E(JIJ, JK) * PETHETA(JIJ, JK) * PTURB_SPP(JIJ,TURBN%ZCEP)
  END DO
END DO
!
...
SUBROUTINE M3_WTH_W2TH(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PKEFF,PTKE,PM3_WTH_W2TH)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE M3_WTH_W2TH(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PKEFF,PTKE,PM3_WTH_W2TH,PTURB_SPP)
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_WTH_W2TH(JIJ, JK) = PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)*1.5/ZWORK1(JIJ, JK) &
...
SUBROUTINE D_M3_WTH_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PKEFF,PTKE,PD_M3_WTH_W2TH_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE D_M3_WTH_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PKEFF,PTKE,PD_M3_WTH_W2TH_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_WTH_W2TH_O_DDTDZ(JIJ, JK) = &
     - PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)*1.5/ZWORK1(JIJ, JK)/(1.+PREDTH1(JIJ, JK))**2 &
     * PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK)*PETHETA(JIJ, JK)  &
     * (1. - 0.5*PREDR1(JIJ, JK)*(1.+PREDR1(JIJ, JK))/PD(JIJ, JK)* &
...
SUBROUTINE M3_WTH_W2R(D,CSTURB,TURBN,PD,PKEFF,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_WTH_W2R)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE M3_WTH_W2R(D,CSTURB,TURBN,PD,PKEFF,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_WTH_W2R,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_WTH_W2R(JIJ, JK) = &
      - PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)*0.75*PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK) &
...
SUBROUTINE D_M3_WTH_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PKEFF,PTKE,PBLL_O_E,PEMOIST,PD_M3_WTH_W2R_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE D_M3_WTH_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PKEFF,PTKE,PBLL_O_E,PEMOIST,PD_M3_WTH_W2R_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
SUBROUTINE M3_WTH_WR2(D,CSTURB,TURBN,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PEMOIST,PDTDZ,PM3_WTH_WR2)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE M3_WTH_WR2(D,CSTURB,TURBN,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PEMOIST,PDTDZ,PM3_WTH_WR2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_WTH_WR2(JIJ, JK) = - PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)& 
                               *0.25*PBLL_O_E(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP)*PEMOIST(JIJ, JK)**2 &
...
<<<<<<< HEAD
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
                               &PBETA,PLEPS,PEMOIST,PD_M3_WTH_WR2_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_WTH_WR2_O_DDTDZ(JIJ, JK) = - PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)& 
                               *0.25*PBLL_O_E(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP)*PEMOIST(JIJ, JK)**2 &
...
SUBROUTINE M3_WTH_WTHR(D,CSTURB,TURBN,PREDR1,PD,PKEFF,PTKE,PSQRT_TKE,PBETA,PLEPS,PEMOIST,PM3_WTH_WTHR)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZM_PHY
=======
SUBROUTINE M3_WTH_WTHR(D,CSTURB,TURBN,PREDR1,PD,PKEFF,PTKE,PSQRT_TKE,PBETA,PLEPS,PEMOIST,PM3_WTH_WTHR,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_WTH_WTHR(JIJ, JK) = &
                       PTURB_SPP(JIJ,TURBN%ZCED)*PKEFF(JIJ, JK)*PEMOIST(JIJ, JK)*ZWORK2(JIJ, JK) &
...
SUBROUTINE D_M3_WTH_WTHR_O_DDTDZ(D,CSTURB,TURBN,PM3_WTH_WTHR,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PD_M3_WTH_WTHR_O_DDTDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_WTH_WTHR_O_DDTDZ(D,CSTURB,TURBN,PM3_WTH_WTHR,PREDTH1,PREDR1,PD,PBLL_O_E,PETHETA,PD_M3_WTH_WTHR_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_WTH_WTHR_O_DDTDZ(JIJ, JK) = &
                    ...
                    /PD(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP)*PBLL_O_E(JIJ, JK)*PETHETA(JIJ, JK)
  END DO
END DO
!
...
=======
SUBROUTINE M3_TH2_W2TH(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PDTDZ,PLM,PLEPS,PTKE,PM3_TH2_W2TH,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_TH2_W2TH(JIJ, JK) = - ZWORK2(JIJ, JK) &
                           * 1.5*PLM(JIJ, JK)*PLEPS(JIJ, JK)/PTKE(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP)
  END DO
END DO
!
...
SUBROUTINE D_M3_TH2_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,OUSERV,PD_M3_TH2_W2TH_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_TH2_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,OUSERV,PD_M3_TH2_W2TH_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
!
IF (OUSERV) THEN
!  D_M3_TH2_W2TH_O_DDTDZ(IIJB:IIJE,1:IKT) = - 1.5*PLM*PLEPS/PTKE*PTURB_SPP(JIJ,TURBN%ZCEP) * MZF(                    &
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PD_M3_TH2_W2TH_O_DDTDZ(JIJ, JK) = - 1.5*PLM(JIJ, JK)*PLEPS(JIJ, JK) &
                                                       /PTKE(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP) * ZWORK2(JIJ, JK)
    END DO
  END DO
...
SUBROUTINE D_M3_TH2_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_TH2_WTH2_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_TH2_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_TH2_WTH2_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_TH2_WTH2_O_DDTDZ(JIJ, JK) = PLEPS(JIJ, JK) & 
                                     *0.5/CSTURB%XCTD/PSQRT_TKE(JIJ, JK)*PTURB_SPP(JIJ,TURBN%ZCEP) * ZWORK2(JIJ, JK)
  END DO
END DO
!
...
SUBROUTINE M3_TH2_W2R(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_W2R)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE M3_TH2_W2R(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_W2R,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_TH2_W2R(JIJ, JK) = 0.75*PTURB_SPP(JIJ,TURBN%ZCEP)**2*ZWORK2(JIJ, JK) &
...
SUBROUTINE D_M3_TH2_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_W2R_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_TH2_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_W2R_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_TH2_W2R_O_DDTDZ(JIJ, JK) = 0.75*PTURB_SPP(JIJ,TURBN%ZCEP)**2*PLM(JIJ, JK) *PLEPS(JIJ, JK) &
...
SUBROUTINE M3_TH2_WR2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_WR2)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE M3_TH2_WR2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_WR2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_TH2_WR2(JIJ, JK) = 0.25*PTURB_SPP(JIJ,TURBN%ZCEP)**2*ZWORK2(JIJ, JK)&
...
SUBROUTINE D_M3_TH2_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_WR2_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_TH2_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_WR2_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_TH2_WR2_O_DDTDZ(JIJ, JK) = 0.25*PTURB_SPP(JIJ,TURBN%ZCEP)**2*PLEPS(JIJ, JK) & 
...
SUBROUTINE M3_TH2_WTHR(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_WTHR)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE M3_TH2_WTHR(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_TH2_WTHR,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_TH2_WTHR(JIJ, JK) = - 0.5*PTURB_SPP(JIJ,TURBN%ZCEP)*PLEPS(JIJ, JK) & 
                                           / PSQRT_TKE(JIJ, JK)/CSTURB%XCTD * ZWORK2(JIJ, JK)
  END DO
END DO
!
...
SUBROUTINE D_M3_TH2_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_WTHR_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_TH2_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_TH2_WTHR_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_TH2_WTHR_O_DDTDZ(JIJ, JK) = - 0.5*PTURB_SPP(JIJ,TURBN%ZCEP)*PLEPS(JIJ, JK) & 
                                                    / PSQRT_TKE(JIJ, JK)/CSTURB%XCTD * ZWORK2(JIJ, JK) 
  END DO
END DO
!
...
SUBROUTINE D_M3_THR_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_THR_WTHR_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_THR_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_THR_WTHR_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_THR_WTHR_O_DDTDZ(JIJ, JK) = 0.5*PLEPS(JIJ, JK)/PSQRT_TKE(JIJ, JK) & 
                                                    / CSTURB%XCTD * PTURB_SPP(JIJ,TURBN%ZCEP) * ZWORK2(JIJ, JK)
  END DO
END DO
!
...
SUBROUTINE M3_THR_WTH2(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_THR_WTH2)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE M3_THR_WTH2(D,CSTURB,TURBN,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_THR_WTH2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PM3_THR_WTH2(JIJ, JK) = - 0.25*PLEPS(JIJ, JK) & 
                                        / PSQRT_TKE(JIJ, JK)/CSTURB%XCTD*PTURB_SPP(JIJ,TURBN%ZCEP) * ZWORK2(JIJ, JK)
  END DO
END DO
!
...
SUBROUTINE D_M3_THR_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_THR_WTH2_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_THR_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_THR_WTH2_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_THR_WTH2_O_DDTDZ(JIJ, JK) = - 0.25*PLEPS(JIJ, JK) &
                                     /PSQRT_TKE(JIJ, JK)/CSTURB%XCTD*PTURB_SPP(JIJ,TURBN%ZCEP)**2 * ZWORK2(JIJ, JK) 
  END DO
END DO
!
...
SUBROUTINE D_M3_THR_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_THR_WTH2_O_DDRDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_THR_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PD_M3_THR_WTH2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
SUBROUTINE M3_THR_W2TH(D,CSTURB,TURBN,PREDR1,PD,PLM,PLEPS,PTKE,PDRDZ,PM3_THR_W2TH)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE M3_THR_W2TH(D,CSTURB,TURBN,PREDR1,PD,PLM,PLEPS,PTKE,PDRDZ,PM3_THR_W2TH,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
SUBROUTINE D_M3_THR_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PDRDZ,PETHETA,PD_M3_THR_W2TH_O_DDTDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_THR_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PDRDZ,PETHETA,PD_M3_THR_W2TH_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PD_M3_THR_W2TH_O_DDTDZ(JIJ, JK) = - 0.75*PLM(JIJ, JK)*PLEPS(JIJ, JK)&
                                                    / PTKE(JIJ, JK) * PTURB_SPP(JIJ,TURBN%ZCEP)**2 * ZWORK1(JIJ, JK)
  END DO
END DO
!
...
SUBROUTINE D_M3_THR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PD_M3_THR_W2TH_O_DDRDZ)
  ...
  USE MODE_SHUMAN_PHY, ONLY: MZF_PHY
=======
SUBROUTINE D_M3_THR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDTH1,PREDR1,PD,PLM,PLEPS,PTKE,PD_M3_THR_W2TH_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
SUBROUTINE M3_WR_WR2(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PM3_WR_WR2)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_WR_WR2(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PM3_WR_WR2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_WTH_WTH2(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PM3_WR_WR2,PTURB_SPP)
!
...
SUBROUTINE D_M3_WR_WR2_O_DDRDZ(D,CSTURB,TURBN,PM3_WR_WR2,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WR2_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_WR_WR2_O_DDRDZ(D,CSTURB,TURBN,PM3_WR_WR2,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WR2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_WTH_WTH2_O_DDTDZ(D,CSTURB,TURBN,PM3_WR_WR2,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WR2_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_WR_W2R(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PM3_WR_W2R)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_WR_W2R(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PM3_WR_W2R,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_WTH_W2TH(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PM3_WR_W2R,PTURB_SPP)
!
...
SUBROUTINE D_M3_WR_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PKEFF,PTKE,PD_M3_WR_W2R_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_WR_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PKEFF,PTKE,PD_M3_WR_W2R_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_WTH_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PKEFF,PTKE,PD_M3_WR_W2R_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_WR_W2TH(D,CSTURB,TURBN,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_WR_W2TH)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_WR_W2TH(D,CSTURB,TURBN,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_WR_W2TH,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_WTH_W2R(D,CSTURB,TURBN,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_WR_W2TH,PTURB_SPP)
!
...
SUBROUTINE D_M3_WR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PD_M3_WR_W2TH_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_WR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PD_M3_WR_W2TH_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_WTH_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PBLL_O_E,PETHETA,PD_M3_WR_W2TH_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_WR_WTH2(D,CSTURB,TURBN,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,PDRDZ,PM3_WR_WTH2)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_WR_WTH2(D,CSTURB,TURBN,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,PDRDZ,PM3_WR_WTH2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_WTH_WR2(D,CSTURB,TURBN,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,PDRDZ,PM3_WR_WTH2,PTURB_SPP)
!
...
<<<<<<< HEAD
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
                               &PBETA,PLEPS,PETHETA,PD_M3_WR_WTH2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_WTH_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PKEFF,PTKE,PSQRT_TKE,PBLL_O_E,PBETA,PLEPS,PETHETA,PD_M3_WR_WTH2_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_WR_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PKEFF,PTKE,PSQRT_TKE,PBETA,PLEPS,PETHETA,PM3_WR_WTHR)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_WR_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PKEFF,PTKE,PSQRT_TKE,PBETA,PLEPS,PETHETA,PM3_WR_WTHR,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_WTH_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PKEFF,PTKE,PSQRT_TKE,PBETA,PLEPS,PETHETA,PM3_WR_WTHR,PTURB_SPP)
!
...
SUBROUTINE D_M3_WR_WTHR_O_DDRDZ(D,CSTURB,TURBN,PM3_WR_WTHR,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WTHR_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_WR_WTHR_O_DDRDZ(D,CSTURB,TURBN,PM3_WR_WTHR,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WTHR_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_WTH_WTHR_O_DDTDZ(D,CSTURB,TURBN,PM3_WR_WTHR,PREDR1,PREDTH1,PD,PBLL_O_E,PEMOIST,PD_M3_WR_WTHR_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_R2_W2R(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PDRDZ,PLM,PLEPS,PTKE,PM3_R2_W2R)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_R2_W2R(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PDRDZ,PLM,PLEPS,PTKE,PM3_R2_W2R,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_TH2_W2TH(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PDRDZ,PLM,PLEPS,PTKE,PM3_R2_W2R,PTURB_SPP)
!
...
SUBROUTINE D_M3_R2_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,OUSERV,PD_M3_R2_W2R_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_R2_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,OUSERV,PD_M3_R2_W2R_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_TH2_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,OUSERV,PD_M3_R2_W2R_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE D_M3_R2_WR2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_R2_WR2_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_R2_WR2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_R2_WR2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_TH2_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_R2_WR2_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_R2_W2TH(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_W2TH)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_R2_W2TH(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_W2TH,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_TH2_W2R(D,CSTURB,TURBN,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_W2TH,PTURB_SPP)
!
...
SUBROUTINE D_M3_R2_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_W2TH_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_R2_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_W2TH_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_TH2_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_W2TH_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_R2_WTH2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTH2)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_R2_WTH2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTH2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_TH2_WR2(D,CSTURB,TURBN,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTH2,PTURB_SPP)
!
...
SUBROUTINE D_M3_R2_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTH2_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_R2_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTH2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_TH2_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTH2_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_R2_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTHR)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_R2_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTHR,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_TH2_WTHR(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PM3_R2_WTHR,PTURB_SPP)
!
...
SUBROUTINE D_M3_R2_WTHR_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTHR_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_R2_WTHR_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTHR_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_TH2_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PETHETA,PDRDZ,PD_M3_R2_WTHR_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE D_M3_THR_WTHR_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WTHR_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_THR_WTHR_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WTHR_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_THR_WTHR_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WTHR_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE M3_THR_WR2(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_THR_WR2)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_THR_WR2(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_THR_WR2,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_THR_WTH2(D,CSTURB,TURBN,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PM3_THR_WR2,PTURB_SPP)
!
...
SUBROUTINE D_M3_THR_WR2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_THR_WR2_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_THR_WR2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_THR_WR2_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_THR_WTH2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PDTDZ,PD_M3_THR_WR2_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE D_M3_THR_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WR2_O_DDTDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_THR_WR2_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WR2_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_THR_WTH2_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLEPS,PSQRT_TKE,PBLL_O_E,PEMOIST,PD_M3_THR_WR2_O_DDTDZ,PTURB_SPP)
!
...
SUBROUTINE M3_THR_W2R(D,CSTURB,TURBN,PREDTH1,PD,PLM,PLEPS,PTKE,PDTDZ,PM3_THR_W2R)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE M3_THR_W2R(D,CSTURB,TURBN,PREDTH1,PD,PLM,PLEPS,PTKE,PDTDZ,PM3_THR_W2R,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL M3_THR_W2TH(D,CSTURB,TURBN,PREDTH1,PD,PLM,PLEPS,PTKE,PDTDZ,PM3_THR_W2R,PTURB_SPP)
!
...
SUBROUTINE D_M3_THR_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PDTDZ,PEMOIST,PD_M3_THR_W2R_O_DDRDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_THR_W2R_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PDTDZ,PEMOIST,PD_M3_THR_W2R_O_DDRDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_THR_W2TH_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PBLL_O_E,PDTDZ,PEMOIST,PD_M3_THR_W2R_O_DDRDZ,PTURB_SPP)
!
...
SUBROUTINE D_M3_THR_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PD_M3_THR_W2R_O_DDTDZ)
  ...
  USE MODD_DIMPHYEX,       ONLY: DIMPHYEX_t
=======
SUBROUTINE D_M3_THR_W2R_O_DDTDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PD_M3_THR_W2R_O_DDTDZ,PTURB_SPP)
>>>>>>> c625cc3a (Add turbulence and shallow convection SPP.)
  TYPE(DIMPHYEX_t),                   INTENT(IN) :: D
  TYPE(CSTURB_t),                     INTENT(IN) :: CSTURB
  TYPE(TURB_t),                    INTENT(IN)    :: TURBN
...
CALL D_M3_THR_W2TH_O_DDRDZ(D,CSTURB,TURBN,PREDR1,PREDTH1,PD,PLM,PLEPS,PTKE,PD_M3_THR_W2R_O_DDTDZ,PTURB_SPP)
!
